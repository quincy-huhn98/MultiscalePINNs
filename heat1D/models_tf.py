...
class heat1D_NN:
    def __init__(self, layers, operator, k, q, bcs_sampler, res_sampler, sigma, X_star, u_star):
        X, _ = res_sampler.sample(np.int32(1e3))
        self.mu_X, self.sigma_X = X.mean(0), X.std(0)
        #self.mu_t, self.sigma_t = self.mu_X[0], self.sigma_X[0]
        self.mu_x, self.sigma_x = self.mu_X, self.sigma_X
    ...
    def net_r(self, x):
        u = self.net_u(x)
        residual = self.operator(u, x, self.k, self.sigma_X)
        return residual
    ...
    def train(self, nIter=10000, batch_size=128):
        ...
        for it in range(nIter):
            ...
            if it % 100 == 0:
                ...
                loss_bcs_value, loss_res_value = self.sess.run([self.loss_bcs,
